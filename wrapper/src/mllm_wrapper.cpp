...
void mllmWrapper::loadModel(std::string model_dir) {
    ...
    switch (model_type) {
        case Qwen2: {
            vocab_path = model_dir + "/qwen2.5_vocab.mllm";
            merge_path = model_dir + "/qwen2.5_merges.txt";
        ...
        case Llama3: {
            vocab_path = model_dir + "/llama3_tokenizer.model";
            model_path = model_dir + "/model.mllm";
...
std::vector<int> mllmWrapper::tokenizeInternal(const std::string& text) {
    ...
    switch(model_type) {
        ...
        case Llama3: {
            auto tensor = ((LLama3Tokenizer*)(tokenizer.get()))->tokenize(text);
            for (int i = 0; i < tensor.sequence(); ++i) {
                auto value = tensor.dataAt<float>(0, 0, i, 0);
                tokens_id.push_back(value);
            }
